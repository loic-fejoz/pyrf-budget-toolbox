...
```{python}
ap_tx_cable = Loss(name='AP\nCable', loss=dB(2))
ap_antenna = Antenna(name='AP\nAntenna', gain=dB(10))
path = FreeSpacePathLossFriis(distance=km(5), freq=GHz(2.4))
client_antenna = Antenna(name='Client\nAntenna', gain=dB(14))
client_rx_cable = Loss(name='Client\nCable', loss=dB(2))
...
_svg_ = downlink.schemdraw({"simplified": False, "with_nf": False})
```
