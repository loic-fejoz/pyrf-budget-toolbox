...
import math
from rfbudget import GHz, m, km, dB, dBm, Antenna, Loss, FreeSpacePathLossFriis, budget, Orbit, degree, MHz, Cable, watt_to_dBm, kelvin
...
# Receiver noise temperature (referred to LNA input)
G_lna_lin = 10**(G_lna_dB / 10)
T_rcvr = T_lna + T_stage2 / G_lna_lin

# System noise temperature (referred to antenna input)
T_sys = Ta * L + T0 * (1-L) + T_rcvr

table_noise = [
    ["Transmission Line Loss Factor (L)", f"{L:.3f}", ""],
    ["Receiver Noise Temp (Trx)", f"{T_rcvr:.2f}", "K"],
    ["System Noise Temp (Tsys)", f"{T_sys:.2f}", "K"]
]
...
# Temperatures
Ta = kelvin(110) # Antenna T°
T0 = kelvin(290) # Feedline T°
T_lna = kelvin(3) # LNA T°
G_lna_dB = dB(25) # LNA gain
T_fe = kelvin(500) # Front-end T°

# Receiver noise temperature (referred to LNA input)
#   LNA temperature contribution: T_lna
#   Post-LNA cable contribution (at LNA output): T0 * (1 - G_post) / G_post
#   Receiver front-end contribution (at cable input): T_fe / G_post
#   Sum referred to LNA input: T_lna + (T0 * (1 - G_post) / G_post + T_fe / G_post) / G_lna

G_lna_lin = 10**(G_lna_dB / 10)
post_lna_cable_loss_dB = 25 * 0.092
G_post_lin = 10**(-post_lna_cable_loss_dB / 10)

T_rcvr = T_lna + (T0 * (1 - G_post_lin) / G_post_lin + T_fe / G_post_lin) / G_lna_lin

# Transmission line loss factor L (linear gain factor)
L = 10**(-total_downlink_rx_line_loss / 10)

# System noise temperature (referred to LNA input)
Ts = Ta * L + T0 * (1 - L) + T_rcvr

table_downlink_rx = [
    ["Total In-line Losses", f"{total_downlink_rx_line_loss:.3f}", "dB"],
    ["Transmission Line Loss Factor (L)", f"{L:.3f}", ""],
    ["Receiver Noise Temp (Trx)", f"{T_rcvr:.2f}", "K"],
    ["System Noise Temp (Ts)", f"{Ts:.2f}", "K"]
]
